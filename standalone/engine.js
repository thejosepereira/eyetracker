...
(function (root) {
  ...
  function buildPsf(blur, psfType) {
    if (psfType === "gaussian") {
    ...
    const ksize = Math.max(Math.ceil(Math.max(blur.sigmaX, blur.sigmaY) * 4 * 1.2) | 1, 3);
    return { psf: generateDiskPsf(ksize, 2 * blur.sigmaX, 2 * blur.sigmaY, blur.angle), ksize };
  }
  ...
  function render(rgba, W, H, prescription, opts) {
    ...
    const built = buildPsf(blur, opts.psfType);
    ...
    if (precomp && opts.method === "tv") {
      ...
      for (let y = 0; y < PH; y++) for (let x = 0; x < PW; x++) {
        ...
        const r = 0.5 + (srgbToLinear(rgba[o] / 255) - 0.5) * dr;
        const g = 0.5 + (srgbToLinear(rgba[o + 1] / 255) - 0.5) * dr;
        const b = 0.5 + (srgbToLinear(rgba[o + 2] / 255) - 0.5) * dr;
